...
def getSegmentCandidates(point, segments, maxdist=50): #changed from 10 to 50
    ...
    with open("c:/python27/learnpython/testTrack.shp", "rb") as gpsPoints:
        gpsPoints = [line.rstrip() for line in gpsPoints]
        points = gpsPoints.read()
    #print points
    with open("c:/python27/learnpython/testSegments.shp", "rb") as segments:
        segments = [line.rstrip() for line in segments]
    #print segments
    candidates = {}
...
def getNetworkTransP(s1, s2, segments):
    ...
    points = trackImport() #changed from 'p1', 'p2' etc to a test track
...
def mapMatch(points, segments):
    ...
    V = [{}]
    #init first point
    sc = getSegmentCandidates('Id'[0], 'OBJECTID') #!!!!!!
    #print sc
    for s in sc:
...
points = open("c:/python27/learnpython/testTrack.shp", "rb") #changed from 'p1', 'p2' etc to a test track
segments = open("c:/python27/learnpython/testSegments.shp", "rb") #changed from 's1', 's2' etc to a test segments
